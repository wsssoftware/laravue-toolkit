const multiselectUpdate = function (button, classElement, classToggler) {
...
const multiselectRemoveTag = function (button, classElement, classToggler) {
...
const multiselectSearch = function (event, input, classElement, classToggler, creatable) {
...
const multiselectClear = function (button, classElement) {
    const target = button.closest(`.${classElement}`).previousElementSibling
    Array.from(target.options).forEach(option => option.selected = false)
    target.dispatchEvent(new Event('change'))
}
const multiselect = function (el, option = {}) {
    el.style.display = 'none'
    const classElement = 'multiselect-wrapper'
    const classNoResults = 'multiselect-no-results'
    const classTag = 'multiselect-tag'
    const classTagRemove = 'multiselect-tag-remove'
    const classPlaceholder = 'multiselect-placeholder'
    const classClearBtn = 'multiselect-clear'
    const classTogglerClearable = 'multiselect-clearable'
    const defaultSearch = false
    const defaultCreatable = false
    const defaultClearable = false
    const defaultMaxHeight = '360px'
    const defaultSize = ''
    const search = option.search || defaultSearch
    const creatable = option.creatable || defaultCreatable
    const clearable = option.clearable || defaultClearable
    ...
    const searchInput = search ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="multiselectSearch(event, this, '${classElement}', '${classToggler}', ${creatable})" type="text" class="form-control" placeholder="Search" autofocus>` : ''
    ...
    function selectedTag(options, multiple) {
        if (multiple) {
            ...
            <div class="${classTag}" data-dselect-value="${option.value}">
              ${option.text}
              <svg onclick="multiselectRemoveTag(this, '${classElement}', '${classToggler}')" class="${classTagRemove}" width="14" height="14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>
            </div>
          `)
                }
            }
    ...
    function itemTags(options) {
        ...
        for (const option of options) {
            if (option.tagName === 'OPTGROUP') {
                ...
                items.push(`<button${hidden} class="dropdown-item${active}" data-dselect-value="${value}" type="button" onclick="multiselectUpdate(this, '${classElement}', '${classToggler}')"${disabled}>${text}</button>`)
            }
        }
        items = items.join('')
        return items
    ...
    <button type="button" class="btn ${classClearBtn}" title="Clear selection" onclick="multiselectClear(this, '${classElement}')">
